 ...
 * 功能说明：测试自定义函数
 * author:liangyy
 * createtime：2022-12-29 10:23:10
 */
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Event> stream3 = clicks.filter(new FilterFunction<Event>() {
            ...
            public boolean filter(Event value) throws Exception {
                return value.getUrl().contains("home");
            }
        });

        // 3. 传入Lambda表达式
        SingleOutputStreamOperator<Event> stream4 = clicks.filter(data -> data.getUrl().contains("home"));
...
//        stream3.print();
    ...
    public static class KeyWordFilter implements FilterFunction<Event> {
        ...
        @Override
        public boolean filter(Event value) throws Exception {
            return value.getUrl().contains(this.keyWord);
        }
    }
