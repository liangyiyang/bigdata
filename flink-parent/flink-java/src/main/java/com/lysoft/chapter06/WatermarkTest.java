...
//                .countWindow(10, 5) // 计数-滑动窗口
                .windowAll(GlobalWindows.create()) // 全局窗口，keyBy之后的全局窗口，每个key只有1个窗口，全局窗口没有时间的概念，不会关闭窗口，不会触发计算，必须定义触发器才会执行计算。该全局窗口与非KeyedStream的全局窗口区别，非KeyedStream的全局窗口只有1个并行度。
                ;
